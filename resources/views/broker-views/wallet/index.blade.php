...
@section('title',translate('messages.broker').' '.translate('messages.wallet'))
...
<div class="content container-fluid">
    <div class="card">
        <div class="card-header">
            <h4 class="page-header-title text-capitalize">{{translate('messages.broker')}} {{translate('messages.wallet')}}</h4>
        </div>
        <div class="card-body row">
        <?php
            $wallet = \App\Models\BrokerWallet::where('broker_id',\App\CentralLogics\Helpers::get_broker_id ())->first();
            if(isset($wallet)==false){
                \Illuminate\Support\Facades\DB::table('broker_wallets')->insert([
                    'broker_id'=>\App\CentralLogics\Helpers::get_broker_id(),
                    'created_at'=>now(),
                    'updated_at'=>now()
                ]);
                $wallet = \App\Models\BrokerWallet::where('broker_id',\App\CentralLogics\Helpers::get_broker_id())->first();
            ...
            <div class="for-card col-md-4 mb-1">
                <div class="card for-card-body-2 shadow h-100 text-white"  style="background: #8d8d8d;">
                    ...
                    <div class="card-footer"  style="background: #8d8d8d; border:none;">
                        @if(\App\CentralLogics\Helpers::get_broker_data()->account_no==null || \App\CentralLogics\Helpers::get_broker_data()->bank_name==null)
    ...
    <div class="modal fade" id="balance-modal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('broker.wallet.withdraw-request')}}" method="post">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($wr->approved==0)
                                            ...
                                            <form action="{{route('broker.wallet.close-request',[$wr['id']])}}"
